...
- (void)exitChatroomWithUserMode:(NTESUserMode)userMode {
    [_playerManager stop];
    [[NIMAVChatSDK sharedSDK].netCallManager leaveMeeting:_dataSource.meeting];
    __weak typeof(self) weakSelf = self;
...
- (void)reserveMeeting {
    __weak typeof(self) wself = self;
    [[NIMAVChatSDK sharedSDK].netCallManager reserveMeeting:_dataSource.meeting
                                                 completion:^(NIMNetCallMeeting * _Nonnull meeting, NSError * _Nullable error) {
         if (error) {
             NELPLogError(@"[demo] 预定meeting失败。%@", error);
             [NTESChatroomAlertView showAlertWithMessage:@"预定音视频房间失败" completion:^{
...
- (void)joinMeeting {
    __weak typeof(self) wself = self;
    [[NIMAVChatSDK sharedSDK].netCallManager joinMeeting:_dataSource.meeting
                                              completion:^(NIMNetCallMeeting * _Nonnull meeting, NSError * _Nullable error) {
          if (error) {
    ...
    switch (micInfo.micStatus) {
        ...
        case NTESMicStatusConnectFinished:{
            ...
            //恢复语音
                ...
                [[NIMAVChatSDK sharedSDK].netCallManager setAudioSendMute:NO];
        ...
        case NTESMicStatusConnectFinishedWithMasked: {
            ...
            if (!_dataSource.isMasked) {
                ...
                [[NIMAVChatSDK sharedSDK].netCallManager setMeetingRole:YES];
                [[NIMAVChatSDK sharedSDK].netCallManager setAudioSendMute:YES];
...
- (void)didSoundMute:(BOOL)mute {
    ...
    [[NIMAVChatSDK sharedSDK].netCallManager setAllUserAudioMute:mute];
...
- (void)didUpdateChatroomQueueWithMicInfokey:(NSString *)key micInfoValue:(NSString *)value
{
    ...
    if ([micInfo.userInfo.account isEqualToString:_dataSource.myAccountInfo.account]) {
        if (micInfo.micOrder == _dataSource.myMicInfo.micOrder
            || micInfo.micReason == NTESMicReasonConnectInvited) { //被抱麦或者麦位号一致
...
- (void)onUserJoined:(NSString *)uid
    ...
    BOOL isMyAccount = [uid isEqualToString:_dataSource.myAccountInfo.account];
    if (_dataSource.userMode == NTESUserModeAnchor && !isMyAccount) {
        if (_dataSource.isAllSoundMute) {
...
- (void)onSpeakingUsersReport:(nullable NSArray<NIMNetCallUserInfo *> *)report {
    ...
    [_dataSource.onSoundUsers enumerateObjectsUsingBlock:^(NIMNetCallUserInfo * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        for (NIMNetCallUserInfo *info in report) {
            if ([info.uid isEqualToString:obj.uid]) { //存在了
                isExistInReport = YES;
                [dic setObject:@(1) forKey:obj.uid];
                break;
            }
        }
        if (!isExistInReport) { //不存在就关了
            if ([weakSelf.dataSource userIsCreator:obj.uid]) {
                [weakSelf.headerView stopSoundAnimation];
            } else {
