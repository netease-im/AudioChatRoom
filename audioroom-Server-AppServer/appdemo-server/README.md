...
数据用户名：默认设置为demouser
数据库密码：默认设置为demopass
创建数据表：demo_seq、demo_quiz_room、demo_quiz_question、demo_quiz_game、demo_quiz_tourist以及demo_quiz_answer_stats
具体操作如下：
...
logback.xml文件配置
指定工程日志路径
默认配置为：<property name="log.dir" scope="context" value="${catalina.home}/logs"/> 可以根据需要自定义工程日志路径
3.3 项目部署
通过war包部署
切换到工程根目录下，执行以下操作：
...
接下来就可以将war包发布到已有tomcat的webapps目录下进行部署
直接通过maven tomcat插件部署
项目父工程pom.xml文件中已经定义好了插件配置，默认部署到tomcat根目录下，并指定端口8088。
 <plugin>
...
1.3 返回值
{  "code": 200,  "data": {    "total": 1,    "list": [      {        "roomId": 60935727,        "name": "临湖小筑",        "creator": "user56977",        "thumbnail": "https://s2.ax1x.com/2019/01/15/FzNb5V.png",        "onlineUserCount": 0,        "createTime": 1547134737215      }    ]  }}
2. 获取IM账号
2.1 请求说明
...
2.3 返回值
{    "code": 200,    "data": {        "sid": "user11850333", // sid        "accid": "user11850333", // 用户名        "nickname": "用户468820", // 昵称        "icon": "https://s2.ax1x.com/2019/01/15/FzUSbR.png",        "imToken": "b4b7a5e1730fcd51c1b0ca843640fe9b", // im token        "availableAt": 1547155366875 // 账号过期时间    }}
2.4 错误码
900、911
3. 创建房间
3.1 请求说明
...
3.3 返回值
{    "code": 200,    "data": {        "roomId": 60935727,        "creator": "user11850333",        "name": "临湖小筑",        "thumbnail": "https://s2.ax1x.com/2019/01/15/FzNb5V.png",        "createTime": 1547134737215    }}
3.4 错误码
400、401、800
4. 全员禁言
4.1 请求说明
...
6.3 返回值
{    "code": 200,    "data": "dissolve 5 rooms"}
6.4 错误码
400、403
7. 错误码表
错误码 含义
